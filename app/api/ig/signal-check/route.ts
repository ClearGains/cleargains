...
// 10-minute server cache per epic — protects IG data allowance and Claude costs
...
// A position should be closed when the AI sees a high-confidence reversal.
// HOLD / WATCH = no action. Confidence < 7 = not decisive enough to close.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Return cached result if still fresh (re-stamp shouldClose for this direction)
    ...
    const spreadPct = ((offer - bid) / mid * 100).toFixed(3);

    const prompt = `You are a financial analyst deciding whether to EXIT an open spread-bet position.

Instrument: ${instrumentName ?? epic} (${epic})
Current: bid=${bid}, offer=${offer}, mid=${mid.toFixed(4)}, spread=${spreadPct}%
...
{"signal":"BUY"|"SELL"|"HOLD"|"WATCH","confidence":<1-10>,"reasoning":"<2 sentences on current direction and momentum>"}`;
    ...
    const signal     = (parsed.signal ?? 'HOLD') as RawSignal;
    const confidence = Math.min(10, Math.max(1, parsed.confidence ?? 5));
