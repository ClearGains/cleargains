...
const strategyRunner_1 = require("./strategyRunner");
const botAccount_1 = require("./botAccount");
...
app.get('/health', (_req, res) => {
    res.json({
        ...
        demo: botAccount_1.demoBot.status().running,
        live: botAccount_1.liveBot.status().running,
    });
});
// ── Multi-account routes (/accounts/:account/*) ───────────────────────────────
// GET /accounts — status of both accounts
app.get('/accounts', auth, (_req, res) => {
    res.json({
        demo: botAccount_1.demoBot.status(),
        live: botAccount_1.liveBot.status(),
    });
});
function resolveBot(req, res) {
    const key = req.params.account;
    if (key !== 'demo' && key !== 'live') {
        res.status(400).json({ ok: false, error: 'account must be "demo" or "live"' });
        return null;
    }
    return (0, botAccount_1.getAccountBot)(key);
}
// GET /accounts/:account/status
app.get('/accounts/:account/status', auth, (req, res) => {
...
app.post('/accounts/:account/start', auth, (req, res) => {
    const bot = resolveBot(req, res);
    if (!bot)
        return;
    const { epics, config } = req.body;
...
app.post('/accounts/:account/stop', auth, (req, res) => {
    const bot = resolveBot(req, res);
    if (!bot)
        return;
    bot.stop();
...
app.post('/accounts/:account/pause', auth, (req, res) => {
    const bot = resolveBot(req, res);
    if (!bot)
        return;
    bot.pause();
...
app.post('/accounts/:account/resume', auth, (req, res) => {
    const bot = resolveBot(req, res);
    if (!bot)
        return;
    bot.resume();
...
app.get('/prices', auth, (_req, res) => {
    ...
    for (const bot of [botAccount_1.demoBot, botAccount_1.liveBot]) {
        ...
        for (const [epic, candles] of Object.entries(allCandles)) {
            ...
            const rsi = candles.length >= 14 ? (0, scalperStrategy_1.calcRsi)(candles) : null;
            const macd = candles.length >= 26 ? (0, scalperStrategy_1.calcMacdHist)(candles) : null;
            const atr = candles.length >= 14 ? (0, scalperStrategy_1.calcAtr)(candles) : null;
            ...
            if (candles.length >= 10) {
                ...
                if (last5 > prev5 * 1.0003)
                    trend5m = 'UP'; // tighter threshold for 5-min candles
                else if (last5 < prev5 * 0.9997)
                    trend5m = 'DOWN';
            }
            prices[epic] = {
...
app.post('/start', auth, (req, res) => {
    ...
    void (0, bot_1.startBot)({ epics, config }).then(result => res.json(result));
});
app.post('/stop', auth, (_req, res) => { (0, bot_1.stopBot)(); res.json({ ok: true }); });
app.post('/pause', auth, (_req, res) => { (0, bot_1.pauseBot)(); res.json({ ok: true }); });
app.post('/resume', auth, (_req, res) => { (0, bot_1.resumeBot)(); res.json({ ok: true }); });
// /debug/inject removed — bot no longer executes trades
// ── Strategy runner routes ────────────────────────────────────────────────────
app.post('/strategy/start', auth, (req, res) => {
    const cfg = req.body;
    if (!cfg.markets?.length) {
        res.status(400).json({ ok: false, error: 'markets array is required' });
        return;
    }
    const result = (0, strategyRunner_1.startStrategyRunner)(cfg);
    res.status(result.ok ? 200 : 400).json(result);
});
app.post('/strategy/stop', auth, (_req, res) => { (0, strategyRunner_1.stopStrategyRunner)(); res.json({ ok: true }); });
app.get('/strategy/status', auth, (_req, res) => { res.json((0, strategyRunner_1.getStrategyRunnerStatus)()); });
// ── Start ─────────────────────────────────────────────────────────────────────
app.listen(PORT, '0.0.0.0', () => {
    ...
    const saved = (0, bot_1.loadSavedState)();
    if (saved) {
        console.log(`[bot-server] Auto-resuming legacy bot with ${saved.epics.length} epic(s)...`);
        void (0, bot_1.startBot)(saved).then(r => {
            if (r.ok)
                console.log('[bot-server] Auto-resume successful');
            else
                console.error(`[bot-server] Auto-resume failed: ${r.error}`);
        });
    }
});
