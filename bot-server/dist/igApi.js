...
function headers(session, version = '1') {
    return {
...
async function authenticate(apiKey, username, password, env, accountKey) {
    const base = BASE[env];
...
async function openPosition(session, epic, size, direction = 'BUY', stopLevel, limitLevel) {
    ...
    if (!r.ok)
        throw new Error(`openPosition failed ${r.status}: ${d.errorCode ?? JSON.stringify(d)}`);
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
        ...
        confirm = await cr.json();
        if (cr.ok && confirm.dealStatus && confirm.dealStatus !== 'REJECTED')
            break;
        if (confirm.dealStatus === 'REJECTED')
            break; // definitive — no point retrying
    }
...
async function closePosition(session, dealId, direction, size) {
    const base = BASE[session.env];
    ...
    for (const attempt of attempts) {
        const hdrs = headers(session, attempt.version);
        if ('override' in attempt && attempt.override)
            hdrs['_method'] = 'DELETE';
        ...
        if (r.status !== 404 && r.status !== 405)
            throw new Error(`closePosition failed ${r.status}: ${d.errorCode ?? attempt.url}`);
        // otherwise fall through to next attempt
    }
    throw new Error(`closePosition: all endpoints returned 404 for dealId=${dealId}`);
}
async function fetchCandleHistory(session, epic, resolution = 'MINUTE_5', count = 35) {
    const base = BASE[session.env];
...
async function fetchPositions(session) {
    const base = BASE[session.env];
    const endpoints = [
