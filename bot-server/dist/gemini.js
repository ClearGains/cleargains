...
function fallbackVerdict(signal) {
...
async function askGemini(signal) {
    const apiKey = process.env.GEMINI_API_KEY;
    // Stage 1: rules filter — always runs first
    const rules = fallbackVerdict(signal);
    if (!apiKey || rules.direction === 'SKIP')
        return rules;
    // Stage 2: Gemini second opinion — only reached if rules say enter
    const candleStr = signal.lastCandles.map((c, i) => `  [${i + 1}] O=${c.open.toFixed(2)} H=${c.high.toFixed(2)} L=${c.low.toFixed(2)} C=${c.close.toFixed(2)} ${c.close >= c.open ? '▲' : '▼'}`).join('\n');
...
{"direction":"BUY","confidence":72,"reason":"max 12 words","stopPoints":0.0012,"takeProfitPoints":0.0018,"betSize":0.5}`;
    try {
        ...
        const dir = (['BUY', 'SELL', 'SKIP'].includes(parsed.direction)) ? parsed.direction : 'SKIP';
