...
const FOREX_BATCH_SIZE = 3;   // CS.D.* forex — each batch gets its own subscription
const INDEX_BATCH_SIZE = 6;   // IX.D.* indices — can share; they rarely tick anyway
...
export function connect(session: IGSession, epics: string[], onTick: OnTickFn, resolution = '1MINUTE') {
  ...
  // Forex and index epics get separate subscriptions — mixing them causes
  // silent data loss on IG's Lightstreamer when inactive index items are
  // batched together with active forex pairs
  subscribeBatches(forexEpics, FOREX_BATCH_SIZE);
  subscribeBatches(indexEpics, INDEX_BATCH_SIZE);

  console.log(`[igStream] Subscribed to ${epics.length} epic(s): ${forexEpics.length} forex in ${Math.ceil(forexEpics.length / FOREX_BATCH_SIZE)} sub(s), ${indexEpics.length} indices in ${Math.ceil(indexEpics.length / INDEX_BATCH_SIZE)} sub(s)`);
}
